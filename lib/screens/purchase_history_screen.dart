  ...
  Widget build(BuildContext context) {
    ...
    final tableContent = _loading
        ...
        : isMobile
            ...
            final purchasesCard = SectionCard(title: 'Purchases', child: SizedBox(height: purchasesHeight, child: ScrollableDataTable(minWidth: isMobile ? 420 : 620, table: DataTable(showCheckboxColumn: false, dataRowColor: WidgetStateProperty.resolveWith((states) { if (states.contains(WidgetState.selected)) { return const Color(0xFF243B63); } return null; }), columns: const [DataColumn(label: Text('Purchase Ref')), DataColumn(label: Text('Total Price')), DataColumn(label: Text('Bought Date')), DataColumn(label: Text('Actions'))], rows: filteredOrders.map((purchase) { final isSelected = purchase['id'] == effectiveSelectedPurchaseId; void selectPurchase() => setState(() => _selectedPurchaseId = purchase['id'] as String?); return DataRow(selected: isSelected, cells: [DataCell(Text(formatReferenceId(purchase['id'], prefix: 'PUR')), onTap: selectPurchase), DataCell(Text(_currency(_purchaseTotal(purchase['id'] as String? ?? ''))), onTap: selectPurchase), DataCell(Text(purchase['bought_date'] ?? ''), onTap: selectPurchase), DataCell(Row(children: [IconButton(icon: const Icon(Icons.edit), onPressed: () => _openPurchaseOrderDialog(purchase: purchase)), IconButton(icon: const Icon(Icons.delete), onPressed: () => _deletePurchaseOrder(purchase['id'] as String))]))]); }).toList()))));
